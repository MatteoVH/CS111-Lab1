...
int get_next_char(command_stream_t cStream) 
{
 int curChar;
 
 //Get char from the command stream if it is not to be ignored
 if(cStream->ignore_char == -2)
   curChar = cStream->getbyte(cStream->arg);
 //Else, we have a character to ignore and mark it as ignored
 else 
 {
   curChar = cStream->ignore_char;
...
void read_next_token(command_stream_t cStream);
{
 ...
 //Checks if there is no more input from the stream
...
command_stream_t make_command_stream (int (*get_next_byte) (void *), void *get_next_byte_argument)
{
 ...
 cStream->next_token_string[0] = 0;
 cStream->current_token_string = checked_malloc ( 50 * sizeof(char) );
 cStream->current_token_string[0] = 0;
 cStream->max_token_length = 50;  //50 characters initially available
